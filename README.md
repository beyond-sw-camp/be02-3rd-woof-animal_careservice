...
1. Woof에 회원가입을 할 수 있다.
   -> e-mail, PW, 닉네임, 전화번호, 반려동물정보를 입력해야 한다.
2. 이메일 인증이 완료된 후 로그인 할 수 있다.
3. 헤더의 메뉴를 통해 업체조회, 매니저조회, 예약페이지, 상세페이지(About Us), 마이페이지로 이동할 수 있다.
4. 업체, 매니저를 선택하여 원하는 시간대에 예약(주문)할 수 있다.

[2] 업체 CEO (유치원, 병원, 미용실)
1. Woof에 회원가입을 할 수 있다.
   -> 사업자등록번호, PW, 이름, 전화번호, 업체소개를 입력해야 한다.
2. 업체 등록을 할 수 있다
   -> 메뉴바에 업체를 클릭하면 업체List로 이동된다. 
   -> CEO 권한이 부여되어있으면 상품등록 버튼이 활성화된다.
   -> 가게이름, 상품이름, 전화번호, 가격, 상품설명, 사진등록 모든 사항을 입력해야 한다.

[3] Woof 매니저
1. Woof에 회원가입을 할 수 있다.
   -> e-mail, PW, 닉네임은 필수로 기입하고, 전화번호, 반려동물정보는 선택적으로 기입 한다.
2. 매니저 등록을 할 수 있다
   -> 메뉴바에 매니저를 클릭하면 매니저List로 이동된다.
   -> 매니저 권한이 부여되어있으면 상품등록 버튼이 활성화된다.
   -> 매니저이름, 성별, 전화번호, 가격, 경력, 사진등록 모든 사항을 입력해야 한다.

[4] 예약
1. 고객은 이름, 전화번호, 예약시간, 특이사항(요구사항), 픽업장소(매니저가 반려동물을 픽업할 장소)를 작성한다.
2. 주문이 완료되면 예약 성공 페이지로 이동한다.
   -> 성공페이지에서는 예약확인(내 주문내역 확인), 예약삭제, 예약수정 버튼을 통해 원하는 서비스가 이용 가능하다.
