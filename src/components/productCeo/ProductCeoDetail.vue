...
export default {
  ...
  methods: {
    ...
    updateProduct() {
      this.$router.push({ name: "productCeoUpdate", query: { idx: this.idx } });
    },
    deleteProduct() {
  if (confirm('정말로 이 상품을 삭제하시겠습니까?')) {
    axios.delete(`http://localhost:8080/productCeo/deleteCeo?idx=${this.idx}`)
      .then(() => {
  ...
  computed: {
    productImageSrc() {
      ...
      //   return `http://localhost:8080/images/${this.product.filenames[0]}`;
      // } else {
      //   return '/static/images/salon.jpg';
      // }
    },
