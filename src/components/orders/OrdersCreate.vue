    ...
    <div>
      <form class="orderform" @submit.prevent >
        ...
        <div class="number">
            ...
            <input type="text" id="productManagerIdx" placeholder="숫자만 써주세요" v-model="productManagerIdx">

            <label>업체 번호
            </label>
            <input type="text" id="productCeoIdx" placeholder="숫자만 써주세요" v-model="productCeoIdx">

            <label>멤버 번호
            </label>
            <input type="text" id="memberIdx" placeholder="숫자만 써주세요" v-model="memberIdx"> 
        </div>
        <!-- 중략 -->
        <input class="submit signup-progress-btn" type="submit" value="전송하기" v-on:click="sendData()">
      </form>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async sendData() {
      ...
      try {
        let response = await axios.post("http://localhost:8080/orders/create", orderDto, {
          headers: {
        ...
        window.location.href = "http://localhost:8081/orders/success"
    }
